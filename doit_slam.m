...
% append a hacky initialization for the position of markers
state_estimate = [state_estimate; reshape(markers_body',[],1)]; 
...
for index = 1:length(markersn)
  ...
  [ marker_error, H ] = predict_markers_slam( quaternion, state_estimate, index, markersn );
...
output = [states_before_measurement(:,1:6) quaternions_before_measurement(:,1:4) states_before_measurement(:,10:12) states_before_measurement(:,13:n_x)];
